#!/usr/bin/env bash
# setup_local_hid.sh — turn the local Linux host into the Bluetooth HID
# mouse/keyboard ("devmouse"), replacing a separate Raspberry Pi.
#
...
#
# Usage (run on the local host, as root):
#   sudo bash scripts/setup_local_hid.sh
#
# Environment overrides (all auto-derived if unset):
#   HID_USER    — the login user that owns the venv        (default: the sudo caller)
...
HID_USER="${HID_USER:-${SUDO_USER:-$(id -un)}}"
...
CONF_BACKUP="$CONF.pre-handsneyes-hid"
OVERRIDE_DIR=/etc/systemd/system/bluetooth.service.d
OVERRIDE=$OVERRIDE_DIR/override.conf
GW_LOG=/tmp/hid-gateway.log
AGENT_LOG=/tmp/hid-agent.log

if [ "$(id -u)" -ne 0 ]; then
    echo "FATAL: run as root (sudo bash scripts/setup_local_hid.sh)" >&2
    exit 1
fi
...
echo "  user=$HID_USER  repo=$REPO_DIR  venv=$GW_VENV  port=$GW_PORT"
...
[ -f "$CONF" ] && [ ! -f "$CONF_BACKUP" ] && cp "$CONF" "$CONF_BACKUP"
cat > "$CONF" <<'BTCONF'
[General]
Name = devmouse
Class = 0x0025C0
...
# this the Mac sees a "headset" and routes its system sound to this host.
...
setsid python3 "$AGENT" >"$AGENT_LOG" 2>&1 < /dev/null &
sleep 1
pgrep -f "bt-agent.py" >/dev/null && echo "  agent up (log: $AGENT_LOG)" \
  || echo "  WARN agent not running — see $AGENT_LOG"

echo "[6/6] Starting afferent gateway as root (binds L2CAP PSM 17/19)..."
pkill -f "afferent-gateway" 2>/dev/null || true
sleep 1
setsid "$GW_BIN" >"$GW_LOG" 2>&1 < /dev/null &
sleep 3
if ss -ltnp 2>/dev/null | grep -q ":$GW_PORT"; then
  echo "  gateway up on :$GW_PORT (log: $GW_LOG)"
else
  echo "  WARN gateway not listening — see $GW_LOG"
fi

echo
echo "=== DONE (stage 1). Now, on the Mac: pair a NEW device 'devmouse'. ==="
echo "    Gateway log:  tail -f $GW_LOG"
echo "    Agent log:    tail -f $AGENT_LOG"
